...
const { getDynamicSecret } = require('../../utils/generateSecrets');
...
// Инициализация Firebase Client SDK
...
// Функция для генерации токенов с динамическими секретами
const generateTokens = (uid) => {
  const timestamp = Date.now();
  const accessSecret = getDynamicSecret(uid, timestamp);
  const refreshSecret = getDynamicSecret(uid, timestamp + 1000); // Немного другой секрет для refresh
...
// Функция для создания имени из email
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Обновление refresh token и timestamp в Firestore
...
// Экспортируем функцию проверки токена для middleware
